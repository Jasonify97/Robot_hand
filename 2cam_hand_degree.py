...
with mp_hands.Hands(
  ...
  while True:
    success_1, image_1 = cap1.read()
    success_2, image_2 = cap2.read()
    
    image_1 = cv2.cvtColor(cv2.flip(image_1, 1), cv2.COLOR_BGR2RGB)
    image_2 = cv2.cvtColor(cv2.flip(image_2, 1), cv2.COLOR_BGR2RGB)
    ...
    image_height_1, image_width_1, _ = image_1.shape
    image_height_2, image_width_2, _ = image_2.shape

    if results_1.multi_hand_landmarks and results_2.multi_hand_landmarks:
      for hand_landmarks in results_1.multi_hand_landmarks:
        ...
        def joint_degree(down, point, up): # PIP, DIP, TIP 관절
          ...
          # 각도 표시
          draw.text((point.x * image_width_1, point.y * image_height_1-20), angle_deg, font=font,fill=(255, 255, 255))
        ...
        def MCP_joint_degree(down, point, up): # MCP 관절
          ...
          draw.text((point.x * image_width_1, point.y * image_height_1+10), angle_deg_xy, font=font,fill=(255, 255, 255))
          draw.text((point.x * image_width_1, point.y * image_height_1+30), angle_deg_yz, font=font,fill=(255, 255, 255))
      ...
      for hand_landmarks in results_2.multi_hand_landmarks:
        ...
        def joint_degree(down, point, up): # PIP, DIP, TIP 관절
          ...
          # 각도 표시
          draw.text((point.x * image_width_2, point.y * image_height_2-20), angle_deg, font=font,fill=(255, 255, 255))
        ...
        def MCP_joint_degree(down, point, up): # MCP 관절
          ...
          angle_deg_xy = str(round(math.degrees(angle_xy),1)) + "도"
          angle_deg_yz = str(round(math.degrees(angle_yz),1)) + "도"
          draw.text((point.x * image_width_2, point.y * image_height_2+10), angle_deg_xy, font=font,fill=(255, 255, 255))
          draw.text((point.x * image_width_2, point.y * image_height_2+30), angle_deg_yz, font=font,fill=(255, 255, 255))
    ...
    cv2.imshow('MediaPipe Hands_front', image_1)
    cv2.imshow('MediaPipe Hands_side', image_2)
    # esc 클릭시 종료
